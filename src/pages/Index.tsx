...
import { Music, Wine, UtensilsCrossed, Beer, Palette, Tent, MapPin, Calendar, ChevronRight, Star } from "lucide-react";
import { Button } from "@/components/ui/button";
import logo from "@/assets/logo_white.png";
import poster from "@/assets/poster.jpeg";

const iconMap: Record<string, any> = {
  Music, Wine, UtensilsCrossed, Beer, Palette, Tent, MapPin,
};
...
const Index = () => {
  ...
  const fetchData = async () => {
    const [sectionsRes, settingsRes, sponsorsRes] = await Promise.all([
      supabase.from("festival_sections").select("*").eq("is_active", true).order("sort_order"),
      supabase.from("app_settings").select("*"),
      supabase.from("sponsor_banners").select("*").eq("is_active", true).order("sort_order"),
    ]);
  ...
  const showSponsors = settings.sponsors_enabled === "true" && sponsors.length > 0;
        ...
        <div className="relative z-10 px-4 py-12 pb-16">
          ...
          <div className="max-w-lg">
            ...
            <p className="text-xl font-light opacity-90 mb-2">
              {settings.festival_subtitle || "Summer on a Solitary Beach"}
            </p>
            <div className="flex items-center gap-2 mt-4 text-sm opacity-80">
              <Calendar className="w-4 h-4" />
              <span>{settings.festival_dates || "12-13-14 Agosto 2026"}</span>
            </div>
            <div className="flex items-center gap-2 mt-1 text-sm opacity-80">
              <MapPin className="w-4 h-4" />
              <span>{settings.festival_location || "Lamezia Terme, Calabria"}</span>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          {sections.map((section, i) => (
            <button
              ...
              className="bg-card rounded-xl p-5 shadow-card text-left hover:shadow-elevated transition-all hover:-translate-y-1 animate-slide-in-bottom"
      ...
      {showSponsors && (
        <section className="px-4 py-6">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="text-sm font-semibold uppercase tracking-wider text-muted-foreground">Sponsor</h3>
          </div>
          <div className="flex gap-4 overflow-x-auto pb-2 scrollbar-hide">
            {sponsors.map((sponsor) => (
              <a
                ...
                className="flex-shrink-0 bg-card rounded-lg p-3 shadow-card hover:shadow-elevated transition-shadow"
              >
                {sponsor.image_url ? (
                  <img src={sponsor.image_url} alt={sponsor.name} className="h-12 w-auto object-contain" />
                ) : (
